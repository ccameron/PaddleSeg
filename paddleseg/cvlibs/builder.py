...
class Builder(object):
    ...
    def build_component(self, cfg):
        ...
        if "type" not in cfg:
            raise RuntimeError(
                "It is not possible to create a component object from {}, as 'type' is not specified.".format(
                    cfg
                )
            )
        ...
        for key, val in cfg.items():
            if self.is_meta_type(val):
                ...
                params[key] = [
                    self.build_component(item) if self.is_meta_type(item) else item
                    for item in val
                ]
            else:
                params[key] = val
...
class SegBuilder(Builder):
    ...
    def model(self) -> paddle.nn.Layer:
        ...
        if self.config.train_dataset_cfg["type"] not in ["Dataset", "SegDataset"]:
            # check and synchronize the num_classes in model config and dataset class
            assert hasattr(
                self.train_dataset_class, "NUM_CLASSES"
            ...
            if "num_classes" in model_cfg:
                assert model_cfg["num_classes"] == num_classes, (
                    ...
                    "and train_dataset class ({}) ".format(
            ...
            assert hasattr(
                self.train_dataset_class, "IMG_CHANNELS"
    ...
    def optimizer(self) -> paddle.optimizer.Optimizer:
        ...
        if opt_cfg["type"] == "SGD" and "momentum" in opt_cfg:
            ...
            logger.info(
                "If the type is SGD and momentum in optimizer config, "
    ...
    def lr_scheduler(self) -> paddle.optimizer.lr.LRScheduler:
        ...
        if lr_type == "PolynomialDecay":
            iters = (
                self.config.iters - warmup_iters if use_warmup else self.config.iters
            )
            iters = max(iters, 1)
    ...
    def _build_loss(self, loss_name, loss_cfg: dict):
        def _check_helper(loss_cfg, ignore_index):
            if "ignore_index" not in loss_cfg:
                ...
                logger.warning(
                    "Add the `ignore_index` in train_dataset "
                ...
                assert loss_cfg["ignore_index"] == ignore_index, (
                    ...
                    "train_dataset ignore_index = {}".format(
                        loss_cfg["ignore_index"], ignore_index
                    )
                )
        ...
        if self.config.train_dataset_cfg["type"] not in ["Dataset", "SegDataset"]:
            assert hasattr(
                self.train_dataset_class, "IGNORE_INDEX"
    ...
    def test_dataset(self) -> paddle.io.Dataset:
        dataset_cfg = self.config.test_dataset_cfg
        if dataset_cfg != {}:
            self.show_msg("test_dataset", dataset_cfg)
            dataset = self.build_component(dataset_cfg)
            if len(dataset) == 0:
                logger.warning(
                    "The number of samples in test_dataset is 0. Please ensure this is the desired behavior."
                )
            return dataset
    ...
    @cached_property
    def test_dataset_class(self) -> Any:
        dataset_cfg = self.config.test_dataset_cfg
        if dataset_cfg != {}:
            dataset_type = dataset_cfg.get("type")
            return self.load_component_class(dataset_type)
